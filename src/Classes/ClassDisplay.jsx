...
const ClassDisplay = ({ oneClass }) => {
    ...
    const { classImage, className, instructorName, availableSeat, price ,_id} = oneClass;
    ...
    const handleSubmit = () => {
        if (user) {
            ...
            fetch('https://astha-sports-academy-server.vercel.app/selectedClass', {
                method: 'POST',
                headers: {
        ...
        <div>
            {
                <div className={`card card-compact w-96 bg-base-100 shadow-xl ${availableSeat ==0 ? 'bg-red-700' : ''}`}>
                    ...
                    <div className="card-body">
                        ...
                        <p className="text-xl font-semibold">Available Seat: {availableSeat}</p>
                        <p className="text-xl font-semibold">Price: {price}</p>
                        <div className="card-actions justify-end">
                        <button onClick={handleSubmit} disabled={availableSeat == 0 || isAdmin[0] == true|| isInstructor[0] == true} className="btn btn-primary uppercase font-bold text-xl">Select</button>
                        </div>
                    </div>
                </div>
            }
